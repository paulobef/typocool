...
const Settings = (props: SettingsProps) => {
  ...
  const { status, settings } = useSelector(
  ...
  return (
    ...
    >
      ...
      <form onSubmit={formik.handleSubmit}>
        ...
        <InputGroup
          error={formik.errors.fontFamily}
          label={"Font style"}
          css={{ maxWidth: 150 }}
        >
          <Select
      ...
      <Text variant={"subtitle"}>
        Last modified:{" "}
        {settings.lastSaved.isToday()
          ? timeDisplayer(settings.lastSaved)
          : dateDisplayer(settings.lastSaved)}
      </Text>
    </div>
  );
};
