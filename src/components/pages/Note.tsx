  ...
  const updater = (whatToUpdate: "title" | "editor") => (
    ...
    setTimer(
      setTimeout(() => {
        ...
        if (!visited) return;
        dispatch(
          updateNote(id, title, editorState.getCurrentContent(), dayjs())
        );
      }, 0)
    );
    dispatch(
      updateLoadedNote({
        ...
        editorState:
          whatToUpdate === "editor" ? (value as EditorState) : editorState,
    ...
    return (
      ...
      >
        <div css={{ width: 750, marginLeft: 80, marginTop: 50 }}>
          ...
          <Text css={{ width: 200 }} variant={"subtitle"}>
            <Skeleton animated />
          </Text>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div css={{ display: "flex", flexDirection: "row" }}>
        ...
        <div
          onClick={() => dispatch(visitLoadedNote())}
...
function Note(props: NoteScreenProps): JSX.Element {
  ...
  const loadError = useSelector(
    (state: RootState) => state.editor.status.loadError
  );
  useEffect(() => {
    if (props.noteId) {
      try {
